...
class UserService:
    ...
    async def set_role_by_sub(self, *, sub: str, role_id: int, db: AsyncSession) -> None:
        """
        Assign a role to a user identified by the Auth `sub` and try to sync the role
        ...
        try:
            ...
            logger.warning(
                "supabase_role_sync_failed sub=%s role_id=%s err=%s",
                sub,
                role_id,
                e,
            )
    ...
    async def upsert_basics_by_sub(
        ...
        *,
        ...
        """
        Update basic fields (email, display name) for an existing user by `sub`.

        Behavior:
            - No-op if the user does not exist.
            - Commits the session after the upsert.
        """
    ...
    async def list_users_full(self, *, db: AsyncSession) -> List[UserDTO]:
        """
        Return all users with their role code resolved.

        Returns:
            List[UserDTO]: Flat DTOs with role code if available.
        """
    ...
    async def get_user_full_by_sub(self, *, sub: str, db: AsyncSession) -> UserDTO:
        """
        Get a single user by `sub` with role code resolved.
